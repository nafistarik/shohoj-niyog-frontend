...
const mockSessions: InterviewSession[] = [
  {
    ...
    qa_pairs: [
      ...
      },
      {
        ...
        answer: "Use React.memo, useMemo, useCallback, lazy loading, and proper component structure.",
      },
    ],
...
export const mockDB = {
  ...
  sessions: {
    ...
    findByCandidate: (candidateEmail: string): InterviewSession[] => {
      return mockSessions.filter((session) => session.allowed_candidates.includes(candidateEmail))
    },
  ...
  responses: {
    ...
    findBySession: (sessionId: string): CandidateResponse[] => {
      return mockResponses.filter((response) => response.session_id === sessionId)
    },
    create: (responseData: Omit<CandidateResponse, "id">): CandidateResponse => {
    ...
    updateDecision: (responseId: string, decision: CandidateResponse["decision"]): boolean => {
